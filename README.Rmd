...
An Emax regression model is estimated using the `emax_nls()` function, using the `structural_model` argument to specify the exposure variable and the response variable, and the `covariate_model` argument to specify covariates to be estimated for structural parameters (e.g., E0, Emax).
...
The package also provides tools to assist in model-based simulations, using the `simulate()` function. A simple example is shown below. Please see the package documentation for more details.

...
using the `emax_nls()` function, using the `structural_model` argument
to specify the exposure variable and the response variable, and the
`covariate_model` argument to specify covariates to be estimated for
structural parameters (e.g., E0, Emax).
...
For binary outcomes, the package provides the `emax_logistic()`

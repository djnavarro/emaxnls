...
test_that(".emax_once_forward and .emax_once_backward select the expected terms", {
  ...
  expect_equal(sort(.extract_coefficient_names(fwd_mod_0a)), sort(.extract_coefficient_names(mod_1)))
  expect_equal(sort(.extract_coefficient_names(bck_mod_1a)), sort(.extract_coefficient_names(mod_1)))
...
test_that("basic use of forward/backward scm works", {
  ...
  expect_equal(sort(.extract_coefficient_names(bck)), sort(.extract_coefficient_names(mod_1))) # should find the E0 ~ cnt_a term only
})

...
test_that(".emax_add_term and .emax_remove_term update the covariate model", {
  ...
  expect_true("E0_bin_d" %in% .extract_parameter_names(mod_add))
  expect_false("E0_cnt_a" %in% .extract_parameter_names(mod_del))
...
test_that("adding a term and later removing leaves the model substantively unchanged", {
  ...
  expect_equal(.extract_parameter_names(mod_del), .extract_parameter_names(mod))
